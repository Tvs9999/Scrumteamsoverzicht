<template>
  <div v-for="classData in sortedClasses" :key="classData.id" class="class" :class="{ 'active': !areAllUsersPresent('class', classData.id) }" :data-id="classData.id">
    <div class="top">
      ...
      <div class="buttons">
        <div v-if="archived" :class="{ 'present': areAllUsersPresent('class', classData.id), 'absent': !areAllUsersPresent('class', classData.id) }">
    ...
    <div class="scrumteams">
      <div v-for="team in classData.scrumteams" :key="team.id" class="scrumteam" :class="{ 'active': !areAllUsersPresent('scrumteam', team.id) }" :data-id="team.id">
        <div class="top">
          <h3>{{ team.name }}</h3>
          <div class="buttons">
            <form v-if="active" :action="'/archive-scrumteam/' + team.id" method="POST">
...
export default defineComponent({
  ...
  mounted(){
    console.log('active prop value:', this.active);
